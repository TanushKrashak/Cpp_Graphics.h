...
//=========================================
	...
	// Part 1
	float p = pow(ry, 2) - ry*pow(rx, 2) + pow(rx, 2)/4.0; // Initial decision parameter
	do {
		...
		if (p < 0)
			...
			y--;
			p += 2*x*pow(ry, 2) + pow(ry, 2) - 2*y*pow(rx, 2);
		}		
	...
	do {
		...
		y--;
		if (p > 0)
			p += pow(rx, 2) - 2*y*pow(rx, 2);
		else {
			x++;
			p += pow(rx, 2) - 2*y*pow(rx, 2) + 2*x*pow(ry, 2);
		}
	} while (y >= 0);
